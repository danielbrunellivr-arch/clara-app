...
async function initSupabase() {
  ...
  return new Promise((resolve) => {
    // Aspetta lo stato auth — più affidabile di getSession al primo caricamento
    const { data: { subscription } } = supabaseClient.auth.onAuthStateChange(async (event, session) => {
      subscription.unsubscribe(); // ascolta solo una volta
      if (session) {
    ...
    // Timeout di sicurezza — se dopo 3 secondi non arriva nulla, continua
