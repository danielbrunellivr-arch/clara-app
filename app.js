...
function getMsgLog() {
  try { const r = localStorage.getItem('clara_msglog'); return r ? JSON.parse(r) : {date:'',count:0}; } catch { return {date:'',count:0}; }
}
...
function incrementMsgCount() {
  ...
  localStorage.setItem('clara_msglog', JSON.stringify({date:today,count}));
}
...
async function submitAuth() {
  ...
  if (authMode === 'signup') {
    // Nuovo utente — mostra sempre il test
  ...
  // Utente esistente — controlla se ha già fatto il test
...
function appendMessage(role, text, ts) {
  ...
  div.innerHTML = `<div class="message-avatar">${init}</div><div><div class="message-bubble">${text.replace(/\n/g,'<br>')}</div><div class="message-time">${getDateTime(ts)}</div></div>`;
