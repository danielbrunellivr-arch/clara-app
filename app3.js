...
async function deleteAccount() {
  if (!confirm('Sei sicuro di voler eliminare il tuo account? Tutti i tuoi dati verranno cancellati definitivamente.')) return;
  if (!confirm("Questa azione è irreversibile. Confermi di voler eliminare l'account?")) return;
  
  try {
    // Elimina dati utente da Supabase
    if (currentUser) {
      await supabaseClient.from('conversazioni').delete().eq('user_id', currentUser.id);
      await supabaseClient.from('esercizi_completati').delete().eq('user_id', currentUser.id);
