...
async function loadProfile() {
  ...
  if (data) {
    ...
    // Aggiorna sidebar con nome e stile
    updateSidebarUser();
  }
}
...
async function markExerciseComplete(exerciseId) {
  ...
  if (!existing) {
    ...
    completedExercisesCache.add(exerciseId);
  }
}
...
function updateCounter() {
  ...
  if (rem === 0) {
    ...
    // Mostra messaggio umano di Clara solo se non già mostrato oggi
    const limitKey = 'clara_limit_shown_' + getTodayStr();
    if (!localStorage.getItem(limitKey)) {
      localStorage.setItem(limitKey, '1');
      const limitMsg = "Ho esaurito i messaggi di oggi per te. Con il piano gratuito puoi scrivere 3 messaggi al giorno, e i tuoi si resettano a mezzanotte.\n\nSe vuoi continuare senza interruzioni, puoi passare a Clara Essenziale. Altrimenti ci rivediamo domani. Prenditi cura di te. ✦";
      appendMessage('clara', limitMsg);
    }
  } else {
...
function updateOnboardingWelcome() {
  const titleEl = document.getElementById('ob-welcome-title');
  if (titleEl && userProfile?.nome) {
    titleEl.textContent = `Ciao ${userProfile.nome}, sono Clara.`;
  }
}

function startApp() {
  const onbEl = document.getElementById('onboarding');
  if (onbEl) { onbEl.style.display = 'none'; onbEl.classList.add('hidden'); }
  updateSidebarUser();
  // Salva stile su Supabase
  if (currentUser) saveProfile({ stile_attaccamento: userStyle, trial_start: new Date().toISOString() });
  const data = STYLES_DATA[userStyle];
  const nome = userProfile?.nome ? `, ${userProfile.nome}` : '';
  const welcome = `Ciao${nome}. ${data.emoji}\n\nHo letto le tue risposte e il tuo profilo emergente è quello ${data.title.toLowerCase()}. Capisco quanto possa essere faticoso portare certi pattern nelle relazioni.\n\nSono qui per te, senza fretta. Puoi iniziare raccontandomi qualcosa che senti in questo momento, oppure chiedermi di guidarti in un esercizio. ✦`;
  const welcomeTs = Date.now();
  appendMessage('clara', welcome, welcomeTs);
  conversationHistory.push({role:'assistant',content:welcome,ts:welcomeTs});
  saveMessageToCloud('assistant', welcome, welcomeTs);
  saveState();
  updateCounter();
}

function updateSidebarUser() {
  const data = userStyle ? STYLES_DATA[userStyle] : null;
  ...
  if (userStatusEl) userStatusEl.textContent = data ? `${data.emoji} ${data.title}` : 'In esplorazione ✦';
}
...
let completedExercisesCache = new Set();

async function refreshCompletedExercises() {
  const completed = await loadCompletedExercises();
  completedExercisesCache = new Set(completed);
  updateCompletedExercisesUI();
}

function updateCompletedExercisesUI() {
  document.querySelectorAll('[data-exercise-id]').forEach(card => {
    const id = card.dataset.exerciseId;
    if (completedExercisesCache.has(id)) {
      if (!card.querySelector('.ex-done-badge')) {
        const badge = document.createElement('span');
        badge.className = 'ex-done-badge';
        badge.style.cssText = 'display:inline-block;background:rgba(124,232,168,0.25);color:#085041;font-size:10px;font-weight:500;padding:2px 8px;border-radius:20px;letter-spacing:0.05em;margin-left:8px;vertical-align:middle';
        badge.textContent = '✓ Fatto';
        // Cerca il primo div con testo (titolo) dentro la card
        const titleEl = card.querySelector('div > div') || card.firstElementChild;
...
function exOpenChat(stepIdx) {
  ...
  setTimeout(async () => {
    showView('chat');
    // Invia il messaggio automaticamente
    const area = document.getElementById('messagesArea');
    if (area) area.scrollTop = area.scrollHeight;
    
    // Mostra il messaggio dell'utente
    const userTs = Date.now();
    appendMessage('user', fullMsg, userTs);
    conversationHistory.push({role:'user', content:fullMsg, ts:userTs});
    saveMessageToCloud('user', fullMsg, userTs);
    
    // Mostra typing e chiama Clara
    showTyping();
    ...
    try {
      ...
      const resp = await fetch('https://super-truth-5867.danielbrunellivr.workers.dev/', {
        ...
        body: JSON.stringify({
          ...
          system: `Sei Clara, una compagna empatica ed esperta in psicologia dell'attaccamento. L'utente ha appena completato un esercizio guidato e sta condividendo quello che è emerso. Accogli con calore quello che ha vissuto, rispecchia brevemente quello che emerge e fai una domanda aperta per approfondire. Max 4-5 righe. Non usare mai il trattino lungo.`,
      ...
      removeTyping();
      const reply = data.content?.[0]?.text || 'Grazie per aver condiviso. Come ti senti adesso?';
      const claraTs = Date.now();
      appendMessage('clara', reply, claraTs);
      ...
      removeTyping();
      appendMessage('clara', 'Grazie per aver condiviso questo con me. Come ti senti adesso?');
    }
  ...
  if (!authEl) {
    ...
    authEl.innerHTML = `
      <div style="background:white;border-radius:28px;padding:48px;max-width:480px;width:100%;box-shadow:0 8px 48px rgba(44,40,37,0.12);border:1px solid var(--light-border);animation:slideUp 0.5s ease">
        ...
        <div id="forgotPwdLink" style="text-align:center;margin-top:10px">
          <button onclick="sendPasswordReset()" style="background:none;border:none;font-family:'DM Sans',sans-serif;font-size:11px;color:var(--warm-gray);cursor:pointer;opacity:0.6;text-decoration:underline" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.6'">Hai dimenticato la password?</button>
        </div>
      </div>`;
...
async function sendPasswordReset() {
  ...
  if (!email) {
    // Mostra istruzioni chiare se l'email non è inserita
    if (errorEl) {
      errorEl.style.background = 'rgba(196,137,122,0.08)';
      errorEl.style.color = 'var(--warm-gray)';
      errorEl.innerHTML = "<strong>Come reimpostare la password:</strong><br>1. Inserisci la tua email nel campo sopra<br>2. Clicca di nuovo 'Hai dimenticato la password?'<br>3. Riceverai una email con il link per creare una nuova password";
  ...
  if (error) {
    if (errorEl) {
      errorEl.style.background = 'rgba(160,101,90,0.08)';
      errorEl.style.color = 'var(--deep-rose)';
      errorEl.textContent = "Errore nell'invio. Controlla che l'email sia corretta e riprova.";
      errorEl.style.display = 'block';
    }
  } else {
    if (errorEl) {
      errorEl.style.background = 'rgba(124,232,168,0.1)';
      errorEl.style.color = '#085041';
      errorEl.innerHTML = 'Email inviata a <strong>' + email + '</strong>.<br>Controlla la tua casella (anche lo spam) e clicca il link per creare una nuova password.';
      errorEl.style.display = 'block';
    }
  }
}
...
async function submitAuth() {
  ...
  if (isNewUser || !profileHasStyle) {
    ...
    conversationHistory = [];
    const onbEl = document.getElementById('onboarding');
    if (onbEl) { onbEl.style.display = 'flex'; onbEl.classList.remove('hidden'); }
...
async function logout() {
  ...
  userStyle = null;
  localStorage.clear();
  // Rimuovi la schermata auth esistente per ricrearla pulita
...
function deleteAccount() {
  ...
  // Mostra modale con istruzioni
  const el = document.createElement('div');
  el.style.cssText = 'position:fixed;inset:0;background:rgba(44,40,37,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:24px';
  el.innerHTML = `
    <div style="background:white;border-radius:24px;padding:36px;max-width:440px;width:100%;box-shadow:0 8px 48px rgba(44,40,37,0.2)">
      <h3 style="font-family:'Cormorant Garamond',serif;font-size:22px;font-weight:400;color:var(--charcoal);margin-bottom:12px">Elimina account</h3>
      <p style="font-size:14px;color:var(--warm-gray);line-height:1.6;margin-bottom:20px">
        Per eliminare completamente il tuo account e tutti i dati associati, scrivici una email a:<br><br>
        <a href="mailto:privacy@clara.app?subject=Richiesta eliminazione account&body=Ciao, vorrei eliminare il mio account associato all'email: ${email}" 
           style="color:var(--rose);font-weight:500">privacy@clara.app</a><br><br>
        Il tuo account verrà eliminato entro 48 ore dalla richiesta.
      </p>
      <div style="display:flex;gap:12px">
        <button onclick="this.closest('div[style]').remove()" style="flex:1;padding:12px;background:var(--cream);border:1px solid var(--light-border);border-radius:12px;font-family:'DM Sans',sans-serif;font-size:13px;cursor:pointer;color:var(--charcoal)">Annulla</button>
        <a href="mailto:privacy@clara.app?subject=Richiesta eliminazione account&body=Ciao, vorrei eliminare il mio account associato all'email: ${email}" 
           style="flex:1;padding:12px;background:var(--charcoal);color:var(--cream);border:none;border-radius:12px;font-family:'DM Sans',sans-serif;font-size:13px;cursor:pointer;text-align:center;text-decoration:none;display:block">
          Scrivi email →
        </a>
      </div>
    </div>`;
...
function showPasswordResetScreen() {
  ...
  const el = document.createElement('div');
  el.id = 'resetPasswordScreen';
  el.style.cssText = 'position:fixed;inset:0;background:var(--cream);z-index:9000;display:flex;align-items:center;justify-content:center;padding:24px';
  el.innerHTML = `
    <div style="background:white;border-radius:28px;padding:48px;max-width:480px;width:100%;box-shadow:0 8px 48px rgba(44,40,37,0.12);border:1px solid var(--light-border)">
      <div style="font-family:'Cormorant Garamond',serif;font-size:36px;font-weight:300;color:var(--charcoal);margin-bottom:4px">Clara</div>
      <div style="font-size:16px;font-family:'Cormorant Garamond',serif;color:var(--charcoal);margin-bottom:8px;margin-top:24px">Crea una nuova password</div>
      <div style="font-size:13px;color:var(--warm-gray);margin-bottom:24px">Scegli una password sicura di almeno 8 caratteri.</div>
      <div style="position:relative;margin-bottom:12px">
        <input id="newPassword" type="password" placeholder="Nuova password" style="width:100%;padding:14px 48px 14px 16px;border:1.5px solid var(--light-border);border-radius:14px;font-family:'DM Sans',sans-serif;font-size:14px;color:var(--charcoal);background:var(--warm-white);outline:none;box-sizing:border-box;transition:border-color 0.2s" onfocus="this.style.borderColor='var(--rose)'" onblur="this.style.borderColor='var(--light-border)'"/>
      </div>
      <div id="resetError" style="display:none;font-size:12px;color:var(--deep-rose);margin-bottom:12px;padding:10px 14px;background:rgba(160,101,90,0.08);border-radius:10px"></div>
      <button onclick="submitNewPassword()" style="width:100%;padding:16px;background:var(--charcoal);color:var(--cream);border:none;border-radius:14px;font-family:'DM Sans',sans-serif;font-size:15px;cursor:pointer;transition:all 0.2s" onmouseover="this.style.background='var(--deep-rose)'" onmouseout="this.style.background='var(--charcoal)'" id="resetBtn">Salva nuova password →</button>
    </div>`;
  document.body.appendChild(el);
}
...
async function submitNewPassword() {
  ...
  if (!pwd || pwd.length < 8) {
    if (errorEl) { errorEl.textContent = 'La password deve essere di almeno 8 caratteri.'; errorEl.style.display = 'block'; }
    return;
  }
  ...
  if (error) {
    if (errorEl) { errorEl.textContent = 'Errore nel salvataggio. Riprova.'; errorEl.style.display = 'block'; }
    btn.textContent = 'Salva nuova password →';
    btn.disabled = false;
  } else {
    document.getElementById('resetPasswordScreen')?.remove();
    // Ricarica la sessione e vai alla chat
    await loadProfile();
    const state = loadState();
...
// ─── INIT ───
async function init() {
  // Rimuovi service worker se presente
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.getRegistrations().then(regs => regs.forEach(r => r.unregister()));
  }

  // Controlla se l'utente arriva da un link di reset password
  ...
  if (isReset && hasSession) {
    // Mostra schermata cambio password
  ...
  if (hasSession) {
    ...
    if (profileDone || localDone) {
      ...
      // Non ha fatto il test — mostra l'onboarding
...
function showView(name) {
  ...
  if (name === 'esercizi') setTimeout(() => refreshCompletedExercises(), 500);
...
function showStyle(s) {
  ...
  setTimeout(() => refreshCompletedExercises(), 500);
    ...
    <div class="reflection-card">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${styleExercises.map(([id, ex]) => `
          <div onclick="openExercise('${id}')" data-exercise-id="${id}" style="display:flex;align-items:center;justify-content:space-between;background:var(--cream);border-radius:14px;padding:14px 18px;cursor:pointer;transition:all 0.2s;border:1px solid transparent" onmouseover="this.style.background='white';this.style.borderColor='${data.color}40';this.style.transform='translateX(4px)'" onmouseout="this.style.background='var(--cream)';this.style.borderColor='transparent';this.style.transform='none'">
            <div>
...
function appendMessage(role, text, ts) {
  ...
  const cleanText = text.replace(/^\[.*?alle \d{2}:\d{2}\]\s*/i, '').replace(/^\[\w.*?\d{2}:\d{2}\]\s*/i, '').trim();
...
async function sendMessage() {
  ...
  // Calcola assenza dall'ultimo messaggio dell'utente
  const prevUserMsgs = conversationHistory.filter(m => m.role === 'user');
  const lastUserTs = prevUserMsgs.length > 1 ? prevUserMsgs[prevUserMsgs.length - 2]?.ts : null;
  const hoursSinceLastMsg = lastUserTs ? (Date.now() - lastUserTs) / (1000 * 60 * 60) : 0;
  const longAbsence = hoursSinceLastMsg >= 72;
  const absenceDays = Math.floor(hoursSinceLastMsg / 24);
...
TONO: Parla in italiano, con calore autentico. Empatica ma mai invadente. Fai domande aperte. Prima accogli, poi esplori. Concisa (max 4-5 righe) salvo richiesta diversa. Non usare mai il trattino lungo (—): usa la virgola, il punto o vai a capo.
...
TEMPO: Ogni messaggio include data e ora. Usale per capire quanto tempo è passato tra i messaggi — se sono passati giorni, Clara deve essere consapevole del salto temporale e non trattare messaggi vecchi come se fossero recenti.${longAbsence ? `
RITORNO: L'utente non scriveva da circa ${absenceDays} giorni. Riconosci questo ritorno in modo naturale e caldo nella tua risposta — senza menzionare i giorni esatti se non suona naturale. Puoi dire qualcosa come "sono contenta di risentirti" o semplicemente mostrare calore extra. Non farne il centro della risposta.` : ''}${comesFromExercise ? `
