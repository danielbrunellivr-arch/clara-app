...
async function sendPasswordReset() {
  ...
  if (!email) {
    if (errorEl) { errorEl.textContent = 'Inserisci prima la tua email.'; errorEl.style.display = 'block'; }
    return;
  }
  ...
  if (error) {
    if (errorEl) { errorEl.textContent = "Errore nell'invio. Riprova."; errorEl.style.display = 'block'; }
  } else {
    if (errorEl) { errorEl.style.background = 'rgba(124,232,168,0.1)'; errorEl.style.color = '#085041'; errorEl.textContent = 'Email inviata! Controlla la tua casella per reimpostare la password.'; errorEl.style.display = 'block'; }
  }
}
